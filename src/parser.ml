...
let parse_tile t = 
  ...
  match t with
  | "grass" -> Grass {color = 0x2ecc71; encounters = [Caterpie; Bulbasaur]}
...
(** [parse_inner_list f j] is a list of [j] parsed according to [f], where [j]
is a Yojson.Basic.t list of strings, essentially for an inner-list of the 2D
list from the JSON file. [f] is the criteria to parse the strings. The order
...
(** [parse_list f j] is the 2D array of the Yojson.Basic.t list [j],
where [j] is a 2D list containing more Yojson.Basic.t lists of values for the
sprite, where [f] is some function to parse the inner values. The order of
[f] and [j] as arguments are for pipelining purposes. *)
let rec parse_list f j = 
  match j with
  | [] -> []
  | h :: t -> (h |> to_list |> parse_inner_list f |> Array.of_list) :: parse_list f t