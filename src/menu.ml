...
let rec help_poke_names (ls : pokemon list) count =
  match ls with
  | [] -> ()
  | h :: t ->
      if count = 8 then () else draw_text poke_name_arr.(count) h.name;
      help_poke_names t (count + 1)
...
let draw_item_menu trainer =
  ...
  draw_item_pokeball ();
  draw_potion_bag ()
let rec help_draw_info pmove_l y_pos = match pmove_l with 
|[] -> y_pos
|h :: t -> draw_text (300, y_pos) (pmove_to_string h); help_draw_info t (y_pos - 18)

let draw_poke_info trainer poke_int = 
  clear_graph();
  fill_draw_rect (0, 0) 500 500 0x2ecc71 black;
  fill_draw_rect (50, 50) 400 400 white black;
  fill_draw_rect (100, 130) 160 240 0x2ecc71 black;
  let poke = (List.nth trainer.team poke_int) in 
  draw_text (300,340) (poke.name);
  let x = help_draw_info poke.moves 300
in draw_text (300, x - 40) ("Health: " ^ string_of_int (poke.stats.health));
draw_text (300, x - 60) ("Level: " ^ string_of_int (poke.stats.level));
draw_text (300, x - 80) ("XP: " ^ string_of_int (poke.stats.xp))
let update_bag key_pressed trainer =
  match key_pressed with
  | 'd' ->
      fill_draw_rect (260, 130) 160 240 0xf1c40f black;
      fill_draw_rect (290, 160) 100 170 white black;
...
let update_menu trainer key_pressed (menu : menu_data) : menu_data =
  ...
  | 'd' -> (
      ...
      | Team ->
          if
            (menu.poke_box >= 0 && menu.poke_box < 3)
            || (menu.poke_box >= 4 && menu.poke_box < 7)
          then (
            redraw_and_fill_boxes (menu.poke_box + 1) trainer;
            { menu with poke_box = menu.poke_box + 1 })
          else menu
      | Bag ->
          update_bag 'd' trainer;
          menu
      | _ -> menu)
  | 'a' -> (
      match menu.menu with
      ...
      | Bag ->
          update_bag 'a' trainer;
          menu
      | _ -> menu)
  | 's' -> (
      match menu.menu with
      | Team ->
          if menu.poke_box < 4 then (
            redraw_and_fill_boxes (menu.poke_box + 4) trainer;
            { menu with poke_box = menu.poke_box + 4 })
          else menu
      | _ -> menu)
  | 'w' -> (
      match menu.menu with
      | Team ->
          if menu.poke_box > 3 then (
            redraw_and_fill_boxes (menu.poke_box - 4) trainer;
            { menu with poke_box = menu.poke_box - 4 })
          else menu
      | _ -> menu)
  | 'e' -> (
      ...
      | MenuTeam ->
          ...
          { menu with menu = Team }
      | Team -> if (menu.poke_box < List.length (trainer.team)) then 
        (draw_poke_info trainer menu.poke_box ;{menu with menu = Poke})
      else menu
      | Bag -> menu
      | _ -> menu)
  | _ -> menu
